...
cor_fatality <-
  df_graph %>%
  group_by(pop_bin) %>%
  summarize(rho = cor(median_income, fatality, method = "spearman", use = "pairwise.complete.obs"))
...
cor_deaths <-
  df_graph %>%
  group_by(pop_bin) %>%
  summarize(rho = cor(median_income, deaths_per100k, method = "spearman", use = "pairwise.complete.obs"))
...
cor_cases <-
  df_graph %>%
  group_by(pop_bin) %>%
  summarize(rho = cor(median_income, cases_per100k, method = "spearman", use = "pairwise.complete.obs"))
...
```{r graphing, fig.width = 16, fig.height = 8}
g_fatality <- df_graph %>% 
  ...
  scale_color_discrete(labels = c("10K-100K", "100K-1M", "1M-10M")) +
  facet_grid(pop_bin ~ ., scales = "free_y") +
