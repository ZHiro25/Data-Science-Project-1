...
1.  Select the **Total Population** table and click the `Download` button to download the data; make sure to select the 2018 5-year estimates.
2.  Unzip and move the data to your `challenges/data` folder.
...
-   The counties with high rates of Covid cases and deaths are all relatively small counties.
-   Half of Chattahoochee, Georgia has gotten Covid.
-   The county with the highest death rate, McMullen, Texas, is one of the smallest counties in the US.
...
```{r}
#Is population correlated with cases in California?
...
p1 <- df_normalized %>% 
  ...
  geom_label_repel(
    data = . %>% filter(county %in% c("San Francisco", "Los Angeles", "Lassen", "Kings", "Imperial")),
    aes(label = county),
    size = 3.1,
    color = "darkblue"
  ) +
  theme_minimal() +
  labs(x = "Population", y = "Cases per 100k Residents")
...
p2 <- df_normalized %>% 
  ...
  ggplot(aes(population, deaths_per100k)) +
  ...
  theme_minimal() +
  labs(x = "Population", y = "Deaths per 100k Residents")
...
-   Population and normalized cases don't seem to be correlated in the state of California.
...
df_normalized %>% 
  ...
  ggplot(aes(cases_per100k, deaths_per100k)) +
  geom_smooth(method = "lm", formula = "y ~ x") +
  geom_point(aes(size = population), color = "cyan3") +
  geom_label_repel(
    data = . %>% filter(county %in% c("San Francisco", "Los Angeles", "Lassen", "Kings", "Imperial")),
    aes(label = county),
    size = 3.1,
  ...
  theme_minimal() +
  labs(x = "Cases per 100k Residents", y = "Deaths per 100k Residents")

df_normalized %>% 
  filter(state == "California") %>%
  summarize(rho = cor(cases_per100k, deaths_per100k, method = "spearman"))
```

**Observations**:

-   Cases and Deaths are linearly correlated for the State of California.
-   The ratio of deaths to cases does not significantly change as cases increase.
-   There are a couple significant outliers. Namely Imperial, Kings, and Lassen counties.
...
df_normalized %>% 
  ...
  ggplot(aes(population, 1000 * deaths_per100k / cases_per100k)) +
  geom_smooth(method = "lm", formula = "y ~ x") +
  geom_point(size = 2, color = "darkorchid1") +
  scale_x_log10(labels = scales::label_number_si()) +
  geom_label_repel(
    data = . %>% filter(county %in% c("San Francisco", "Los Angeles", "Lassen", "Kings", "Imperial", "Inyo", "Calaveras")),
    aes(label = county),
    size = 3.1,
    color = "darkblue"
  ) +
  theme_minimal() +
  labs(x = "Population", y = "Deaths per 1000 Cases")

df_normalized %>% 
  filter(state == "California") %>%
  summarize(rho = cor(population, 1000 * deaths_per100k / cases_per100k, method = "spearman"))
```

**Observations**:

-   Deaths per 1000 cases is poorly correlated with population.
-   The remote counties of Inyo, Calaveras, and Imperial stand out as significant outliers with very high deaths rates relative to the number of cases.


```{r}
pick_state <- "Maine"

state_covid <- df_normalized %>% 
  filter(state == pick_state) %>% 
  group_by(date) %>% 
  summarize(cases = sum(cases), deaths = sum(deaths), population = sum(population)) %>% 
  mutate(new_cases = cases - lag(cases))

state_vaccines <- df_vaccines %>% 
  filter(location == pick_state) %>% 
  arrange(date)

state_data <- inner_join(
  state_covid,
  state_vaccines,
  by = "date"
)
...
coeff <- max(state_data$new_cases)/max(state_data$people_vaccinated_per_hundred, na.rm = TRUE)

state_data %>% 
  filter(new_cases > 0) %>% 
  
  ggplot() +
  geom_point(aes(date, new_cases), alpha = 0.4, size = 2, color = "red", na.rm = TRUE) +
  geom_smooth(aes(date, new_cases), se = FALSE, span = 0.3, size = 1, color = "red", na.rm = TRUE) +
  geom_point(aes(date, people_fully_vaccinated_per_hundred * coeff), alpha = 0.4, size = 2, color = "blue", na.rm = TRUE) +
  geom_smooth(aes(date, people_fully_vaccinated_per_hundred * coeff), se = FALSE, span = 0.3, size = 1, color = "blue", na.rm = TRUE) +
  scale_y_continuous(
    "Daily Case", 
    sec.axis = sec_axis(~ . / coeff, name = "% Vaccinated")
  ) +
  ggtitle(paste("Daily Cases vs Vaccination Rate in", pick_state))


coeff2 <- max(100 * state_data$deaths / state_data$cases) / max(state_data$people_vaccinated_per_hundred, na.rm = TRUE)

state_data %>% 
  filter(new_cases > 0) %>% 
  
  ggplot() +
  geom_point(aes(date, 100 * deaths / cases), alpha = 0.4, size = 2, color = "red", na.rm = TRUE) +
  geom_smooth(aes(date, 100 * deaths / cases), se = FALSE, span = 0.3, size = 1, color = "red", na.rm = TRUE) +
  geom_point(aes(date, people_fully_vaccinated_per_hundred * coeff2), alpha = 0.4, size = 2, color = "blue", na.rm = TRUE) +
  geom_smooth(aes(date, people_fully_vaccinated_per_hundred * coeff2), se = FALSE, span = 0.3, size = 1, color = "blue", na.rm = TRUE) +
  scale_y_continuous(
    "Fatality Rate", 
    sec.axis = sec_axis(~ . / coeff2, name = "% Vaccinated")
  ) +
  ggtitle(paste("Fatality Rate vs Vaccination Rate in", pick_state))
